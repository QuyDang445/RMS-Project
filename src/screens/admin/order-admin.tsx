...
import {AlertYesNo, generateRandomId} from '../../utils';
...
const Category = memo(() => {
	...
	const onPressHandleDelete = (item: CategoryService) => {};
...
const ServiceCategory = memo(() => {
	...
	const onRefresh = async () => {
		setRefreshing(true);
		const res = (await API.get(`${TABLE.CATEGORY}`, true)) as CategoryType[];
		setData(res);
		setRefreshing(false);
	};
		...
		<ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />} style={styles.view}>
			...
			{data.map(item => (
				<View
					...
					<View>
						<CustomText font={FONT_FAMILY.BOLD} text={item.name} />
						<CustomText text={getNameType(item.idCategoryService) || 'Loại dịch vụ'} />
					</View>
					<View style={{flexDirection: 'row', gap: widthScale(10)}}>
						<TouchableOpacity onPress={() => onPressEdit(item)}>
							<Image style={styles.icon} source={ICONS.edit} />
